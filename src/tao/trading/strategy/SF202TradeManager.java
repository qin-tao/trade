...
public class SF202TradeManager extends TradeManager2
{
	SF202 mainProgram;
	boolean CNT = false;
    String TIME_ZONE;
	...
	public SF202TradeManager()
	{
		super();
	}

	public SF202TradeManager(String account, EClientSocket client, Contract contract, int symbolIndex, Logger logger)
	{
			...
			if ( market_order == true )
			{
				if (triggerPrice > entryPrice) 
				{
					...
					if (MODE == Constants.REAL_MODE)
					{
						...
						es.sendEmail(symbol + " BUY order placed", "sent from automated trading system");
					}
				}
			}
			else
		...
		{	
			...
			if ( market_order == true ) 
			{
				if (triggerPrice < entryPrice ) 
				{
					...
					{
						EmailSender es = EmailSender.getInstance();
						es.sendEmail(symbol + " SELL order placed", "sent from automated trading system");
					}
	...
	public void trackPullBackTradeSell(QuoteData data, double price)
	{
		...
		if ( market_order == true ) 
		{
			if (triggerPrice < entryPrice ) 
			{
				...
				{
					EmailSender es = EmailSender.getInstance();
					es.sendEmail("202 Detection", symbol + " SELL detected " + data.time );
				}*/
	...
	public void trackPullBackTradeBuy(QuoteData data, double price )
	{
		...
		if ( market_order == true )
		{
			if (triggerPrice > entryPrice) 
			{
				...
				{
					EmailSender es = EmailSender.getInstance();
					es.sendEmail("202 Detection", symbol + " BUY detected " + data.time );
				}*/
			}
		}
		else

...
public class SF202aTradeManager extends TradeManager2
{
	SF202a mainProgram;
	boolean CNT = false;
    String TIME_ZONE;
...
//	int firstRealTimeDataPosL = 0;
	...
	public SF202aTradeManager()
	{
		super();
	}

	public SF202aTradeManager(String account, EClientSocket client, Contract contract, int symbolIndex, Logger logger)
	{
...
*/
	...
	public Trade checkReallyBreakOut3(QuoteData data )
	{
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);
		int lastbarL = quotesL.length - 1;
		double[] ema20L = Indicator.calculateEMA(quotesL, 20);
		...
		// now it is touching 20MA
		int lastUpPos = Pattern.findLastPriceConsectiveAboveMA(quotesL, ema20L, lastbarL, 2);
		int lastDownPos = Pattern.findLastPriceConsectiveBelowMA(quotesL, ema20L, lastbarL, 2);
	
		if ( lastUpPos > lastDownPos)
		{
			// currently it is on the up side, check up first before check down
			int latestHigh = Utility.getHigh( quotesL, lastDownPos, lastbarL).pos;
			int start = latestHigh;
			Trade t = checkReallyBreakOut3Sell(data, quotesL, start );
			if ( t != null )
				return t;

			// check to see if there is a push to the down side
			int downStart = lastDownPos;
			while ( quotesL[downStart].high < ema20L[downStart])
				downStart--;
			start = Utility.getLow( quotesL, downStart, lastDownPos).pos;
			t = checkReallyBreakOut3Buy(data, quotesL, start );
			if ( t != null )
				return t;
		}
		else if ( lastDownPos > lastUpPos )
		{
			// currently it is on the down side, check up first before check up
			int latestLow = Utility.getLow( quotesL, lastUpPos, lastbarL).pos;
			int start = latestLow;
			Trade t = checkReallyBreakOut3Buy(data, quotesL, start );
			if ( t != null )
				return t;
			...
			while ( quotesL[upStart].low > ema20L[upStart])
				upStart--;
			start = Utility.getHigh( quotesL, upStart, lastUpPos).pos;

			t = checkReallyBreakOut3Sell(data, quotesL, start );
			if ( t != null )
				return t;
	...
	public Trade checkReallyBreakOut3Buy(QuoteData data, QuoteData[] quotesL, int start  )
	{
		int lastbarL = quotesL.length - 1;
		PushList pushList = PushSetup.getUp2PushList(quotesL, start, lastbarL);
		
		if (( pushList != null ) && ( pushList.getNumOfPushes()))
		{
			//System.out.println(symbol + " " + pushList.toString(quotesL, PIP_SIZE));
			...
			//if ( phls.length == 1 )
			{
				if (( phls[lastPushIndex].pushSize > FIXED_STOP * PIP_SIZE ))// &&  (phls[lastPushIndex].pullBackSize < 0.618 * phls[lastPushIndex].pushSize ))
				{
					if ( findTradeHistory( Constants.ACTION_BUY, quotesL[start].time) != null )
						return null;

					for ( int i = phls[lastPushIndex].prePos + 1; i < lastbarL; i++)
						if ( quotesL[i].high > quotesL[phls[lastPushIndex].prePos].high )  // missed
							return null;

					createOpenTrade(Constants.TRADE_EUR, Constants.ACTION_BUY);
					trade.entryPrice = quotesL[phls[lastPushIndex].prePos].high;
					...
					warning("break UP start detected at " + data.time + " start:"+ quotesL[start].time);
					//warning("break UP detected at " + data.time + " start:" + quotesL[start].time + " touch20MA:" + quotesL[touch20MA].time + " breakout tip is " + trade.entryPrice + "@" + firstBreakOut.time + " touch20MA:" + quotesL[touch20MA].time  );
	...
	public Trade checkReallyBreakOut3Sell(QuoteData data, QuoteData[] quotesL, int start )
	{
		int lastbarL = quotesL.length - 1;
		PushList pushList = PushSetup.getDown2PushList(quotesL, start, lastbarL);
		...
		if (( pushList != null ) && ( pushList.getNumOfPushes()))
		{
			...
			//if ( phls.length == 1 )
			{
				if (( phls[lastPushIndex].pushSize > FIXED_STOP * PIP_SIZE ))// &&  (phls[lastPushIndex].pullBackSize < 0.618 * phls[lastPushIndex].pushSize ))
				{
					...
					for ( int i = phls[lastPushIndex].prePos + 1; i < lastbarL; i++)
						if ( quotesL[i].low < quotesL[phls[lastPushIndex].prePos].low )  // missed
							return null;
					
					createOpenTrade(Constants.TRADE_EUR, Constants.ACTION_SELL);
					trade.entryPrice = quotesL[phls[lastPushIndex].prePos].low;
					trade.setEntryTime(quotesL[lastbarL].time);
					//trade.setFirstBreakOutTime(firstBreakOut.time);
					trade.setFirstBreakOutStartTime(quotesL[start].time);
					//trade.setTouch20MATime(quotesL[touch20MA].time);
					...
					warning("break down start detected at " + data.time + " start:"+ quotesL[start].time);
					//warning("break UP detected at " + data.time + " start:" + quotesL[start].time + " touch20MA:" + quotesL[touch20MA].time + " breakout tip is " + trade.entryPrice + "@" + firstBreakOut.time + " touch20MA:" + quotesL[touch20MA].time  );
	...
	public Trade checkReallyBreakOut2(QuoteData data, double lastTick_bid, double lastTick_ask )
	{
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);
		int lastbarL = quotesL.length - 1;
		double[] ema20L = Indicator.calculateEMA(quotesL, 20);
		...
		{
			// check breakout to the upside
		}
		else if ( quotesL[lastbarL].close < quotesL[lastbarL].open )
		{
			// check breakout to the downside
		...
		if ( lastUpPos > lastDownPos)
		{
			...
			start = Utility.getLow(quotesL, prevDownPos, lastUpPos).pos;
			
			//debug("break out start detected at " + quotesL[start].time);
			if ( findTradeHistory( Constants.ACTION_BUY, quotesL[start].time) != null )
				return null;
			...
			*/
			...
			//if ( firstBreakOut != null )
			{
				...
				for ( int i = firstBreakOut.pos+1; i < lastbarL; i++)
					if ( quotesL[i].high > firstBreakOut.high )
					{
						debug("first breakout high missed at " + quotesL[i].time);
						return null;
					}*/
...
//				trade.lastLargeMoveEnd = lastLargeMoveEnd;
		...
		{	
			...
			start = Utility.getHigh(quotesL, prevUpPos, lastDownPos).pos;
			...
			*/
			...
			//if ( firstBreakOut != null )
			{
				...
				for ( int i = firstBreakOut.pos+1; i < lastbarL; i++)
					if ( quotesL[i].low < firstBreakOut.low )
					{
						debug("first breakout low missed at " + quotesL[i].time);
						return null;
					}
...
//				trade.lastLargeMoveEnd = lastLargeMoveEnd;
	...
	public void trackPullBackTradeSell(QuoteData data, double price)
	{
		...
		if ( pushList != null )
		{
			...
			if (( pushSize <=4 ) && ( pushSize > 1 ))
			{
				if (( phls[lastPushInd-1].breakOutSize > 0.5*FIXED_STOP * PIP_SIZE ) && ( phls[lastPushInd].pullBackSize < 1.2 * FIXED_STOP * PIP_SIZE ))
				{
					double entryPrice = quotesL[phls[lastPushInd].prePos].low;
					...
					{	
							...
							||  (Math.abs( triggerPrice - entryPrice) > 7  * PIP_SIZE ))
						{
							...
							trade.openOrderDurationInHour = 3;
							trade.limit1Stop = adjustPrice(entryPrice + FIXED_STOP  * PIP_SIZE, Constants.ADJUST_TYPE_UP);
							enterLimitPosition1(entryPrice, trade.positionSize, trade.limit1Stop); 
							return;
						}
					}*/
	...
	public void trackPullBackTradeBuy(QuoteData data, double price )
	{
		...
		if ( pushList != null )
		{
			...
			if (( pushSize <=4 ) && ( pushSize > 1 ))
			{
				if (( phls[lastPushInd-1].breakOutSize > 0.5*FIXED_STOP * PIP_SIZE ) && ( phls[lastPushInd].pullBackSize < 1.2 * FIXED_STOP * PIP_SIZE ))
				{
					double entryPrice = quotesL[phls[lastPushInd].prePos].high;
	...
	public void exit123_reversal( QuoteData data )
	{
		QuoteData[] quotes = getQuoteData(Constants.CHART_15);
		int lastbar = quotes.length - 1;
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);;

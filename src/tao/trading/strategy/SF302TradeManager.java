...
import tao.trading.trend.analysis.TrendBreak;
import tao.trading.trend.analysis.TrendLine;

import com.ib.client.Contract;
import com.ib.client.EClientSocket;

public class SF302TradeManager extends TradeManager2 implements Runnable
{
	SF302 mainProgram;
	boolean CNT = false;
    String TIME_ZONE;
...
//	int firstRealTimeDataPosL = 0;
	...
	public SF302TradeManager()
	{
		super();
	}

	public SF302TradeManager(String account, EClientSocket client, Contract contract, int symbolIndex, Logger logger)
	{
	...
	public Trade checkPullBacktoBigMoves(QuoteData data)
	{
		...
		double[] ema10_240 = Indicator.calculateEMA(quotes240, 10);

		int pushTip = lastbar240-2;
		if ((quotes240[lastbar240-2].close > ema10_240[lastbar240-2]) && (quotes240[lastbar240-1].high < quotes240[lastbar240-2].high))
		{
			if (( trade != null ) &&  trade.action.equals(Constants.ACTION_BUY))
				return null;

			Push lastPush = Utility.findLargestConsectiveUpBars(quotes240, lastbar240-12, lastbar240-2);
			if (( quotes240[lastPush.pushEnd].close - quotes240[lastPush.pushStart].open ) > 2 * FIXED_STOP * PIP_SIZE )
			{
				if  (findTradeHistory( Constants.ACTION_BUY, quotes240[pushTip].time) != null) 
					return null;

				System.out.println(symbol + " " + CHART + " " + " BUY detected " + quotes240[lastbar240].time);
				...
				trade.setFirstBreakOutTime(quotes240[pushTip].time);
				trade.setFirstBreakOutStartTime(quotes240[pushTip].time);
				return trade;
				
			}

		}
		else if ((quotes240[lastbar240-2].close < ema10_240[lastbar240-2]) && (quotes240[lastbar240-1].low > quotes240[lastbar240-2].low))
		{
			if (( trade != null ) &&  trade.action.equals(Constants.ACTION_SELL))
				return null;

			Push lastPush = Utility.findLargestConsectiveDownBars(quotes240, lastbar240-12, lastbar240-2);
			if (( quotes240[lastPush.pushStart].open - quotes240[lastPush.pushEnd].close ) > 2 * FIXED_STOP * PIP_SIZE )
			{
				if  (findTradeHistory( Constants.ACTION_SELL, quotes240[pushTip].time) != null) 
					return null;

				System.out.println(symbol + " " + CHART + " " + " SELL detected " + quotes240[lastbar240].time);
	...
	public Trade checkPullBack50(QuoteData data)
	{
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);
		QuoteData[] quotes240 = getQuoteData(Constants.CHART_240);
		int lastbarL = quotesL.length - 1;
		...
		int upPos = Pattern.findLastPriceConsectiveAboveMA(quotesL, ema50L, 2);
		int downPos = Pattern.findLastPriceConsectiveBelowMA(quotesL, ema50L, 2);

		//System.out.println("check pullback 20");

		if ((downPos > upPos) && ( downPos == lastbarL-1 ) && ( quotesL[lastbarL].high > ema50L[lastbarL]))
		{
			// pullback from downside;
			...
			if ( trade != null ) 
			{
				...
				else if ( trade.action.equals(Constants.ACTION_BUY))
				{
					...
					while (quotesL[downStart].high < ema50L[downStart])
					{
						downStart--;
						if ( downPos - downStart > 3 )
						{
							trade = null;
							return null;
						}
			...
			/******************************
			// check on big trend
			// this need to determine this is a strong move
			******************************/
			/*
			TrendBreak tb = TrendAnalysis.detectTrendBreak(quotes240);
			if (!(( tb != null ) && (tb.direction == Constants.DIRECTION_DOWN)))
			{
				return null;
			}
			//Push bigMovePush = checkWithBigMove(Constants.DIRECTION_DOWN);
			//if (!(( bigMovePush != null ) && ( bigMovePush.direction == Constants.DIRECTION_DOWN)))
				//return null;*/
			...
			int startL = downPos;
			while (( quotesL[startL].high < ema50L[startL]) && (startL > 0 ))
				startL--;

			int prevConsectiveUpPos = Pattern.findLastPriceConsectiveAboveMA(quotesL, ema50L, startL, 2);
			if (prevConsectiveUpPos == Constants.NOT_FOUND)
				prevConsectiveUpPos = 0;

			int downPos2 = Pattern.findLastPriceConsectiveAboveMA(quotesL, ema50L, startL, 2);
	...
	//			pullBackStart = Utility.getLow( quotesL, pushStartL, lastbarL-1).pos;
			...
			//tb = TrendAnalysis.detectTrendBreak(quotes240);
			createOpenTrade(Constants.TRADE_PBK, Constants.ACTION_SELL);
			trade.status = Constants.STATUS_DETECTED;
			...
			{
				//warning("send notification email for " + " SELL detected " + data.time + " pullback start is at " + " " +  quotes240[tb.breakPoint].time);
				EmailSender es = EmailSender.getInstance();
				es.sendYahooMail("302 Detection", symbol +  " SELL detected " + data.time + " pullback start is at " + " " );
			}

	        return trade;

		}
		else if ((upPos > downPos) && ( upPos == lastbarL-1 ) && ( quotesL[lastbarL].low < ema50L[lastbarL]))
		{
			// pullback from upside;
			if ( trade != null ) 
			{
				...
				{
					int upStart = upPos -1;
					while (quotesL[upStart].low > ema50L[upStart])
					{	
						upStart--;
						if ( upPos - upStart > 3 )
						{
			...
			******************************/
		/*	TrendBreak tb = TrendAnalysis.detectTrendBreak(quotes240);
			if (!(( tb != null ) && (tb.direction == Constants.DIRECTION_UP)))
			{
				return null;
			}*/
			BigTrend bt = determineBigTrend2( quotes240);
			...
			//Push bigMovePush = checkWithBigMove(Constants.DIRECTION_UP);
			//if (!(( bigMovePush != null ) && ( bigMovePush.direction == Constants.DIRECTION_UP)))
			//	return null;

			
			int startL = upPos;
			while (( quotesL[startL].low > ema50L[startL]) && ( startL> 0 ))
				startL--;

			int prevConsectiveDownPos = Pattern.findLastPriceConsectiveBelowMA(quotesL, ema50L, startL, 2);
			if (prevConsectiveDownPos == Constants.NOT_FOUND)
				prevConsectiveDownPos = 0;
			
			int upPos2 = Pattern.findLastPriceConsectiveBelowMA(quotesL, ema50L, startL, 2);
			...
			//logger.warning(symbol + " BUY detected " + data.time + " pullback break is " + quotes240[tb.breakPoint].time );
			//tb = TrendAnalysis.detectTrendBreak(quotes240);
			createOpenTrade(Constants.TRADE_PBK, Constants.ACTION_BUY);
			trade.status = Constants.STATUS_DETECTED;
			...
			if ( MODE == Constants.SIGNAL_MODE )
			{
				//warning("send notification email for " + symbol + " " + CHART + " " + " BUY detected " + data.time + " pullback start is at " + quotesL[pullBackStart].time );
				EmailSender es = EmailSender.getInstance();
				es.sendYahooMail("302 Detection", symbol + " " + CHART + " " + " BUY detected " + data.time + " pullback start is at " + " " );
			}
			return trade;
		}
	...
	public Trade checkPullBack20_with_momentume(QuoteData data)
	{
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);
		int lastbarL = quotesL.length - 1;
		double[] ema20L = Indicator.calculateEMA(quotesL, 20);
		
		int upPos = Pattern.findLastPriceConsectiveAboveMA(quotesL, ema20L, 6);
		int downPos = Pattern.findLastPriceConsectiveBelowMA(quotesL, ema20L, 6);

		if ((downPos > upPos) && ( downPos == lastbarL-1 ) && ( quotesL[lastbarL].high > ema20L[lastbarL]))
		{
			// pullback from downside;
			if (( trade != null ) &&  trade.action.equals(Constants.ACTION_SELL))
					return null;
			...
			/******************************
			// check on big trend 240, the big trend needs to be on below 20MA and pulling back
			******************************/
			QuoteData[] quotes240 = getQuoteData(Constants.CHART_240);
			int lastbar240 = quotes240.length - 1;
			boolean largePushDetected = false;
			for ( int j = lastbar240; j > lastbar240 -5; j--)
			{	
				PushHighLow phl = TrendAnalysis.detectLatestMomentumChange(quotes240, j, 1.5 * FIXED_STOP * PIP_SIZE);
				if ((phl != null ) && (phl.direction == Constants.DIRECTION_DOWN))
				{
					largePushDetected = true;
					break;
				}
			}
			if (!largePushDetected)
				return null;
...
/*			//BigTrend bt = determineBigTrend_v3( quotesL);
			...
			int upPos240 = Pattern.findLastPriceConsectiveAboveMA(quotes240, ema20_240, 2);
			int downPos240 = Pattern.findLastPriceConsectiveBelowMA(quotes240, ema20_240, 2);
			if ( upPos240 > downPos240 )
				return null;
			
			MATouch[] mat = MABreakOutTouchSetup.findMATouchUpsFromGoingDown(quotes240, ema20_240, upPos, lastbar240);
			...
			*/
			...
			logger.warning(symbol + " " + CHART + " " + " sell detected " + data.time + " pullback start is at "  +  quotesL[pullBackStart].time);
			//System.out.println(mbol.toString(quotes240));
			...
			trade.pullBackStartL = pullBackStart; 
			//trade.pullBackPos = pullBackStart;
			addTradeEvent(symbol + " SELL detected " + quotesL[lastbarL].time);
			if ( MODE == Constants.SIGNAL_MODE )
			{
				//warning("send notification email for " + " SELL detected " + data.time + " pullback start is at " + " " +  quotes240[tb.breakPoint].time);
				EmailSender es = EmailSender.getInstance();
		...
		else if ((upPos > downPos) && ( upPos == lastbarL-1 ) && ( quotesL[lastbarL].low < ema20L[lastbarL]))
		{
			// pullback from upside;
			if (( trade != null ) &&  trade.action.equals(Constants.ACTION_BUY))
				return null;

			//System.out.println("check buy at " + quotesL[lastbarL].time);
			int startL = upPos;
			...
			******************************/
			...
			{	
				PushHighLow phl = TrendAnalysis.detectLatestMomentumChange(quotes240, j, 1.5 * FIXED_STOP * PIP_SIZE);
				if ((phl != null ) && (phl.direction == Constants.DIRECTION_UP))
				{
					largePushDetected = true;
					break;
				}
...
*/
			...
			int upPos240 = Pattern.findLastPriceConsectiveAboveMA(quotes240, ema20_240, 2);
			int downPos240 = Pattern.findLastPriceConsectiveBelowMA(quotes240, ema20_240, 2);
			if ( downPos240 > upPos240 )
				return null;
			
			MATouch[] mat = MABreakOutTouchSetup.findMATouchDownsFromGoingUp(quotes240, ema20_240, downPos, lastbar240);
			if (( mat != null ) && ( mat.length > 2 ))
				return null;
			...
			QuoteData[] quotes240 = getQuoteData(Constants.CHART_240);
			double[] ema20_240 = Indicator.calculateEMA(quotes240, 20);
			MABreakOutList mbol = MABreakOutTouchSetup.getMABreakOuts2(quotes240, ema20_240);
*/			
		//	if ( mbol.direction != Constants.DIRECTION_UP )
		//		return null;
			
			
			
			logger.warning(symbol + " BUY detected " + data.time + " pullback break is " );
			//System.out.println(mbol.toString(quotes240));
			//tb = TrendAnalysis.detectTrendBreak(quotes240);
			createOpenTrade(Constants.TRADE_PBK, Constants.ACTION_BUY);
			trade.status = Constants.STATUS_DETECTED;
			...
			trade.detectTime = quotesL[lastbarL].time;
			trade.pullBackStartL = pullBackStart; 
			addTradeEvent(symbol + " BUY detected " + quotesL[lastbarL].time);
			if ( MODE == Constants.SIGNAL_MODE )
			{
				//warning("send notification email for " + symbol + " " + CHART + " " + " BUY detected " + data.time + " pullback start is at " + quotesL[pullBackStart].time );
				EmailSender es = EmailSender.getInstance();
	...
	public Trade checkPullBack20_signal(QuoteData data)
	{
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);
		int lastbarL = quotesL.length - 1;
		double[] ema20L = Indicator.calculateEMA(quotesL, 20);
...
/*			//BigTrend bt = determineBigTrend_v3( quotesL);
		...
		//	double[] ema20_240 = Indicator.calculateEMA(quotes240, 20);
		//	MABreakOutList mbol = MABreakOutTouchSetup.getMABreakOuts2(quotes240, ema20_240);
		//	if ( mbol.direction != Constants.DIRECTION_DOWN )
		//		return null;
...
*/
	...
	public Trade checkPullBack20_3(QuoteData data)
	{
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);
		QuoteData[] quotes240 = getQuoteData(Constants.CHART_240);
		int lastbarL = quotesL.length - 1;
		...
		if ( quotesL[lastbarL].high > ema20L[lastbarL] )
		{
			...
			******************************/
			...
			BigTrend bt = determineBigTrend2( quotesL);
			//if (!( Constants.DIRT_DOWN.equals(bt.direction) || Constants.DIRT_DOWN_REVERSAL.equals(bt.direction) || Constants.DIRT_DOWN_SEC_2.equals(bt.direction)))
			if (! Constants.DIRT_DOWN.equals(bt.direction)) 
				return null;

			Push lastP =  bt.pushes.lastElement();
			if ( lastP.duration < 40 )
				return null;
			...
			int pullBackStart = Utility.getLow( quotesL, prevConsectiveUpPos, downPos).pos;
			int mostPushIndex = -1;
			int lastPushMaxNum = 0;
			PushList mostPush = null;
			PushHighLow lastPush = null;
			
			PushList pushList = PushSetup.getUp2PushList(quotesL, pullBackStart, lastbarL);
			if ((pushList == null ) || ( pushList.phls.length < 1 ))
				return null;
			
			if (pushList.phls[pushList.phls.length-1].curPos != lastbarL)
				return null;
			
			lastPushMaxNum = pushList.phls.length;
			mostPushIndex = pullBackStart;
			mostPush = pushList;
			lastPush = pushList.phls[pushList.phls.length-1];
			
			// looking for the start point that has the most peaks
			for ( int i = pullBackStart+1; i < downPos; i++)
			{
				pushList = PushSetup.getUp2PushList(quotesL, i, lastbarL);
				if (( pushList != null ) && ( pushList.phls.length > 0 ))
				{	
					if ( pushList.phls.length > lastPushMaxNum )
					{
						lastPushMaxNum = pushList.phls.length;
						mostPushIndex = i;
						mostPush = pushList;
			...
			if ( lastPush.pullBackSize < 20 * PIP_SIZE )
				return null;
			
			pullBackStart = mostPushIndex;
			
			if  (findTradeHistory( Constants.ACTION_SELL, quotesL[pullBackStart].time) != null) 
				return null;

			logger.warning(symbol + " " + CHART + " " + " sell detected " + quotesL[lastbarL].time + " pullback start is at "  +  quotesL[pullBackStart].time);
			...
			trade.detectPos = lastbarL;
			trade.setFirstBreakOutTime(quotesL[pullBackStart].time);
			trade.setFirstBreakOutStartTime(quotesL[pullBackStart].time);
	...
	public Trade checkPullBack20_2(QuoteData data)
	{
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);
		QuoteData[] quotes240 = getQuoteData(Constants.CHART_240);
		int lastbarL = quotesL.length - 1;
	...
	public Trade checkPullBack20_org(QuoteData data)
	{
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);
		QuoteData[] quotes240 = getQuoteData(Constants.CHART_240);
		int lastbarL = quotesL.length - 1;
		...
		if ((downPos > upPos) && ( downPos == lastbarL-1 ) && ( quotesL[lastbarL].high > ema20L[lastbarL]))
		{
			...
			if ( trade != null ) 
			{
				...
				{
					int downStart = downPos -1;
					while (quotesL[downStart].high < ema20L[downStart])
					{
						downStart--;
						if ( downPos - downStart > 3 )
						{
	...
	//			pullBackStart = Utility.getLow( quotesL, pushStartL, lastbarL-1).pos;
		...
		else if ((upPos > downPos) && ( upPos == lastbarL-1 ) && ( quotesL[lastbarL].low < ema20L[lastbarL]))
		{
			...
			if ( trade != null ) 
			{
				...
				{
					int upStart = upPos -1;
					while (quotesL[upStart].low > ema20L[upStart])
					{	
						upStart--;
						if ( upPos - upStart > 3 )
						{
	...
	Push checkWithBigMove( int direction )
	{
		QuoteData[] quotes240 = getQuoteData(Constants.CHART_240);
		int lastbar240 = quotes240.length - 1;
		double[] ema20_240 = Indicator.calculateEMA(quotes240, 20);
		
		//int lastUpPos240 = Pattern.findLastPriceConsectiveAboveMA(quotes240, ema20_240, lastbar240, 2);
		//int lastDownPos240 = Pattern.findLastPriceConsectiveBelowMA(quotes240, ema20_240, lastbar240, 2 );
		//if ( lastUpPos240 > lastDownPos240)
			//return null;
		...
		{	
			...
			// 1 look for UpPushes
			int lastDownPos240 = Pattern.findLastPriceConsectiveBelowMA(quotes240, ema20_240, lastbar240, 2 );
			if ( lastDownPos240 == Constants.NOT_FOUND )
				lastDownPos240 = Utility.getLow( quotes240, 0, lastbar240).pos;

			PushList pushList = PushSetup.getUp2PushList(quotes240, lastDownPos240, lastbar240);
			...
			{	
				...
				// look for a push from last pullback
				QuoteData latestHigh = Utility.getHigh( quotes240, lastPush.pullBack.pos, lastbar240-1);
				QuoteData pullBackFromLatestHigh = Utility.getLow( quotes240, latestHigh.pos+1, lastbar240);
				double pullBackSize = latestHigh.high - pullBackFromLatestHigh.low;
				
				if ( pullBackSize < 2 * FIXED_STOP * PIP_SIZE )
				{
					Push p = new Push(lastPush.pullBack.pos, latestHigh.pos);
					p.direction = Constants.DIRECTION_UP;
					return p;
				}
				/*
				Push push = Utility.findLargestConsectiveUpBars(quotes240, lastPush.pullBack.pos, lastbar240);
				if ( push != null )
				{
					double totalPushSize = quotes240[push.pushEnd].high - quotes240[push.pushStart].low;
					if  ( totalPushSize > 1.5 * FIXED_STOP * PIP_SIZE )
					{
						push.direction = Constants.DIRECTION_UP;
						return push;
					}
				}*/
			}
			...
			// 2 look for potential reversal downPushes
			int highestPoint = Utility.getHigh( quotes240, lastDownPos240, lastbar240).pos;
			Push push = Utility.findLargestConsectiveDownBars(quotes240, highestPoint, lastbar240);
			if ( push != null )
			{
				double totalPushSize = quotes240[push.pushStart].high - quotes240[push.pushEnd].low;
				if  ( totalPushSize > 1.2 * FIXED_STOP * PIP_SIZE )
				{
					push.direction = Constants.DIRECTION_DOWN;
					return push;
				}
			}
		}
		...
		{	
			...
			// 1 look for UpPushes
			int lastUpPos240 = Pattern.findLastPriceConsectiveAboveMA(quotes240, ema20_240, lastbar240, 2 );
			if ( lastUpPos240 == Constants.NOT_FOUND )
				lastUpPos240 = Utility.getHigh( quotes240, 0, lastbar240).pos;
			PushList pushList = PushSetup.getDown2PushList(quotes240, lastUpPos240, lastbar240);
			...
			{	
				...
				// look for a push from last pullback
				QuoteData latestLow = Utility.getLow( quotes240, lastPush.pullBack.pos, lastbar240-1);
				QuoteData pullBackFromLatestLow = Utility.getHigh( quotes240, latestLow.pos+1, lastbar240);
				double pullBackSize = pullBackFromLatestLow.high - latestLow.low;
				
				if ( pullBackSize < 2 * FIXED_STOP * PIP_SIZE )
				{
					Push p = new Push(lastPush.pullBack.pos, latestLow.pos);
					p.direction = Constants.DIRECTION_DOWN;
					return p;
				}

				// look for a push from last pullback
				/*
				Push push = Utility.findLargestConsectiveDownBars(quotes240, lastPush.pullBack.pos, lastbar240);
				if ( push != null )
				{
					double totalPushSize = quotes240[push.pushStart].high - quotes240[push.pushEnd].low;
					if  ( totalPushSize > 1.5 * FIXED_STOP * PIP_SIZE )
			...
			// 2 look for potential reversal downPushes
			int lowestPoint = Utility.getLow( quotes240, lastUpPos240, lastbar240).pos;
			Push push = Utility.findLargestConsectiveUpBars(quotes240, lowestPoint, lastbar240);
			if ( push != null )
			{
				double totalPushSize = quotes240[push.pushEnd].high - quotes240[push.pushStart].low;
				if  ( totalPushSize > 1.2 * FIXED_STOP * PIP_SIZE )
	...
	/*
	public void checkBigChartTrendLine()
	{
		QuoteData[] quotes240 = getQuoteData(Constants.CHART_240);
		int lastbar240 = quotes240.length - 1;
...
*/
		...
		int lastbar240 = quotes240.length - 1;
		double[] ema20_240 = Indicator.calculateEMA(quotes240, 20);
		//double[] ema50_240 = Indicator.calculateEMA(quotes240, 50);

		int lastUpPos = Pattern.findLastPriceConsectiveAboveMA(quotes240, ema20_240, lastbar240-1, 2);
		int lastDownPos = Pattern.findLastPriceConsectiveBelowMA(quotes240, ema20_240, lastbar240-1, 2 );
	
		if ( lastUpPos > lastDownPos)
		{
			//if ((quotes240[lastbar240].low < ema20_240[lastbar240]) && ( quotes240[lastbar240 - 1].low > ema20_240[lastbar240 - 1]))
			if ((lastUpPos == (lastbar240 -1)) && (quotes240[lastbar240].low < ema20_240[lastbar240]))
			{
				...
				/*
				MABreakOutList mol = MABreakOutTouchSetup.findMABreakOutsUp(quotes240, ema20_240,-1);
				if ( mol != null ) 
				{
				...
				if ( findTradeHistory( Constants.ACTION_BUY, prevHigh.time/*quotes240[lastbar240].time*/) != null )
					return null;
		...
		else if ( lastDownPos > lastUpPos)
		{
			...
			if ((lastDownPos == (lastbar240 -1)) && (quotes240[lastbar240].high > ema20_240[lastbar240]))
			{
				//BigTrend btt = calculateTrend( quotes240);
	...
	public Trade checkNewHigherHighLowerLow(QuoteData data, double price )
	{
		QuoteData[] quotes240 = getQuoteData(Constants.CHART_240);
		int lastbar240 = quotes240.length - 1;
		double[] ema20_240 = Indicator.calculateEMA(quotes240, 20);

		PushList pl = TrendAnalysis.detectNewHigherHighLowerLow( quotes240, lastbar240);
		if (pl != null )
		{
			if ( pl.direction == Constants.DIRECTION_UP )
			{	
				if ( findTradeHistory( Constants.ACTION_BUY, quotes240[pl.begin].time) != null )
					return null;
				//System.out.println(symbol + " pullback buy detected at " + quotes240[lastbar240].time + " start:" + prevHigh.time);
				System.out.println(symbol + " breakNewHigherHigh detect at " + quotes240[lastbar240].time + " pushStart:" + quotes240[pl.begin].time + " " );
	
				PushHighLow phl = pl.phls[0];
				double entryPrice = quotes240[phl.prePos].high;
				
				createOpenTrade(Constants.TRADE_PBK, Constants.ACTION_BUY);
				trade.status = Constants.STATUS_DETECTED;
				trade.setFirstBreakOutStartTime(quotes240[pl.begin].time);
	
				enterMarketPosition(entryPrice);
	
				return trade;
			}
			else if ( pl.direction == Constants.DIRECTION_DOWN )
			{	
				if ( findTradeHistory( Constants.ACTION_SELL, quotes240[pl.begin].time) != null )
					return null;
				//System.out.println(symbol + " pullback buy detected at " + quotes240[lastbar240].time + " start:" + prevHigh.time);
				System.out.println(symbol + " breakNewLowerLow detect at " + quotes240[lastbar240].time + " pushStart:" + quotes240[pl.begin].time + " " );
	
				PushHighLow phl = pl.phls[0];
				double entryPrice = quotes240[phl.prePos].low;
	...
	public void trackTradeEntry(QuoteData data, double price )
	{
		...
		if (trade != null)
		{
			...
			{
				trackPullBackEntry3( data, price);
	...
	public void trackPullBackEntry3(QuoteData data, double price)
	{
			...
			*/
			...
			{	
				...
				if ( lastbarL == pushList.end )
				{
					...
					emailNotifyEntry( Constants.ENTRY_TIP, quotesL[pushList.end].time );
					addTradeEvent("     TIP entry " + quotesL[lastbarL].time);
				...
				{
					emailNotifyEntry( Constants.ENTRY_LAST_PULLBACK, lastPush.pullBack.time );
					addTradeEvent("     break lastPush low entry " + quotesL[lastbarL].time);
			...
			if ( data.high > ema20L[lastbarL])
			{
				...
				{
					warning("Pullback trade sell triggered at " + data.time + " @ " + data.close);
					//trade.entryPrice = data.close;
			...
			if ( data.low < quotesL[pullBackStartL].low )
			{
				emailNotifyEntry( Constants.ENTRY_FALL_OFF_BOTTOM, quotesL[pullBackStartL].time );
				addTradeEvent("     break pullback low entry " + quotesL[lastbarL].time);
				...
				warning("Pullback trade sell triggered at " + data.time + " pullback: " + quotesL[pullBackStartL].time + " @ " + quotesL[pullBackStartL].low );
		...
		else if (trade.action.equals(Constants.ACTION_BUY))
		{
			...
			{	
				...
				{
					emailNotifyEntry( Constants.ENTRY_LAST_PULLBACK, lastPush.pullBack.time );
					addTradeEvent("     break lastpush high entry " + quotesL[lastbarL].time);
					/*
					BigTrend btt = calculateTrend( quotes240);
					if (!Constants.DIRT_DOWN.equals(btt.direction))
					{
			...
			{
				emailNotifyEntry( Constants.ENTRY_FALL_OFF_BOTTOM, quotesL[pullBackStartL].time );
				addTradeEvent("     break pullback high entry " + quotesL[lastbarL].time);

...
public class SF108aTradeManager extends TradeManager2 implements Serializable
{
	...
	protected SF108aTradeManager largerTimeFrameTraderManager;
	...
	public SF108aTradeManager()
	{
		super();
	}

	public SF108aTradeManager(String account, EClientSocket client, Contract contract, int symbolIndex, Logger logger)
	{
	...
	{
		logger.warning(symbol + " " + CHART + " minute " + " check order filled: " + orderId + " " + filled + " filled");
		...
		else if (orderId == trade.limitId2)
		{
			Object[] quotesL = this.largerTimeFrameTraderManager.getQuoteData();
			int lastbarL = quotesL.length - 1;
	...
	public void checkStopTarget_org(QuoteData data)
	{
		Object[] quotes = getQuoteData();
		int lastbar = quotes.length - 1;
		...
		if (Constants.ACTION_SELL.equals(trade.action))
		{
			if ((trade.stop != 0) && (data.high > trade.stop))
			{
				...
				// placeMktOrder( Constants.ACTION_BUY,
			...
			if ((trade.targetPrice != 0) && (data.low < trade.targetPrice))
			{
				...
				// closePositionByMarket( trade.targetPositionSize,
				// trade.targetPrice);
			...
			/*
			 * if ((trade != null )&&(trade.takeProfit2Price != 0) && ( data.low
			 * < trade.takeProfit2Price )) { logger.info(symbol +
			 * " take profit 2 hit, close " + trade.takeProfit2PositionSize +
			 * " @ " + trade.takeProfit2Price); AddTradeCloseRecord( data.time,
			 * Constants.ACTION_BUY, trade.takeProfit2PositionSize,
			 * trade.takeProfit2Price ); closePositionByMarket(
			 * trade.targetPositionSize, trade.takeProfit2Price ); // should
		...
		else if (Constants.ACTION_BUY.equals(trade.action))
		{
			...
			if ((trade.targetPrice != 0) && (data.high > trade.targetPrice))
			{
				...
				// this.closePositionByMarket( trade.targetPositionSize,
				// trade.targetPrice);
				trade.remainingPositionSize -= trade.targetPositionSize;
				if (trade.remainingPositionSize <= 0)
				{
			...
			/*
			 * if ((trade != null )&&(trade.takeProfit2Price != 0) && (
			 * data.high > trade.takeProfit2Price )) { logger.info(symbol +
			 * " take profit 2 hit, close " + trade.takeProfit2PositionSize +
			 * " @ " + trade.takeProfit2Price); AddTradeCloseRecord( data.time,
			 * Constants.ACTION_SELL, trade.takeProfit2PositionSize,
			 * trade.takeProfit2Price); closePositionByMarket(
			 * trade.targetPositionSize, trade.takeProfit2Price); // should have
			 * everything closed }
			 */
	...
	void processAfterHitStopLogic_c()
	{
		...
		if ((trade.type == Constants.TRADE_RST) || (trade.type == Constants.TRADE_RSC)) // RSC
																						// is
																						// not
																						// supported
																						// for
	...
	public Trade checkHigherHighLowerLowSetup2(QuoteData data, Object[] quotesL)
	{
		Object[] quotes = getQuoteData();
		int lastbar = quotes.length - 1;
		int lastbarL = quotesL.length - 1;
		...
		{
			int touch20MA = lastbar;
			while (((QuoteData) quotes[touch20MA]).low > ema20[touch20MA])
				touch20MA--;

			int begin = Pattern.findLastPriceConsectiveBelowMA(quotes, ema20, 3);

			if (begin == Constants.NOT_FOUND)
				return null;

			Object[] peaks = Pattern.findPastHighPeaksAboveMA(quotes, ema20, begin, lastbar);
			/*
			...
			if ((peaks == null) || (peaks.length < 2) || (((Peak) peaks[peaks.length - 1]).pullbackStartPos == 0))
				return null;

			int lastPeakPos = peaks.length - 2;
			QuoteData lastLow = Utility.getLow(quotes, ((Peak) peaks[lastPeakPos]).pullbackStartPos, ((Peak) peaks[lastPeakPos]).pullbackEndPos);
			int thisPeakPos = peaks.length - 1;
			QuoteData thisLow = Utility.getLow(quotes, ((Peak) peaks[thisPeakPos]).pullbackStartPos, ((Peak) peaks[thisPeakPos]).pullbackEndPos);

			// needs to have quotes close below 20MA on last2
			if (thisLow.low < lastLow.low)
			{
				trade = new Trade(symbol);
				trade.type = Constants.TRADE_RST;
				trade.action = Constants.ACTION_SELL;
				trade.POSITION_SIZE = POSITION_SIZE;
				trade.detectTime = ((QuoteData) quotes[lastbar]).time;
				trade.detectPrice = lastLow.low;
				trade.detectPos = lastbar;
				trade.status = Constants.STATUS_OPEN;

				// place order
				trade.orderId = placeMktOrder(Constants.ACTION_SELL, trade.POSITION_SIZE);
				trade.price = lastLow.low;
				trade.entryPrice = trade.price;
				trade.status = Constants.STATUS_PLACED;
				trade.remainingPositionSize = trade.POSITION_SIZE;
				trade.entryTime = ((QuoteData) quotes[lastbar]).time;
				trade.entryPos = lastbar;
				AddTradeOpenRecord(trade.type, ((QuoteData) quotes[lastbar]).time, Constants.ACTION_SELL, trade.POSITION_SIZE, trade.price);
				logger.warning(symbol + " " + CHART + " trade detected SELL at " + data.time + "@" + trade.price);

				trade.stop = Utility.getHigh(quotes, lastLow.pos, lastbar).high;
		...
		{
			int touch20MA = lastbar;
			while (((QuoteData) quotes[touch20MA]).high < ema20[touch20MA])
				touch20MA--;
			...
			Object[] peaks = Pattern.findPastLowPeaksBelowMA(quotes, ema20, begin, lastbar);

			if ((peaks == null) || (peaks.length < 2) || (((Peak) peaks[peaks.length - 1]).pullbackStartPos == 0))
				return null;

			int lastPeakPos = peaks.length - 2;
			QuoteData lastHigh = Utility.getHigh(quotes, ((Peak) peaks[lastPeakPos]).pullbackStartPos, ((Peak) peaks[lastPeakPos]).pullbackEndPos);
			int thisPeakPos = peaks.length - 1;
			QuoteData thisHigh = Utility.getHigh(quotes, ((Peak) peaks[thisPeakPos]).pullbackStartPos, ((Peak) peaks[thisPeakPos]).pullbackEndPos);

			if (peaks.length >= 2)
			{
				// System.out.println(data.time + " " + peaks.length + " " +
				// symbol + " " + CHART + " this high is " + thisHigh.high +
				// " last high is " + lastHigh.high);
			}

			// needs to have quotes close below 20MA on last2
			if (thisHigh.high > lastHigh.high)
			{
				for (int i = peaks.length - 1; i >= 0; i--)
				{
					logger.warning(symbol + " " + CHART + " peak " + i + " high between "
							+ ((QuoteData) quotes[((Peak) peaks[i]).highlowStartPos]).time + "-"
							+ ((QuoteData) quotes[((Peak) peaks[i]).highlowEndPos]).time);
				...
				trade.POSITION_SIZE = POSITION_SIZE;
				trade.detectTime = ((QuoteData) quotes[lastbar]).time;
				trade.detectPrice = lastHigh.high;
				trade.detectPos = lastbar;
				trade.status = Constants.STATUS_OPEN;

				// place order
				trade.orderId = placeMktOrder(Constants.ACTION_BUY, trade.POSITION_SIZE);
				trade.price = lastHigh.high;
				trade.entryPrice = trade.price;
				trade.status = Constants.STATUS_PLACED;
				trade.remainingPositionSize = trade.POSITION_SIZE;
				trade.entryTime = ((QuoteData) quotes[lastbar]).time;
				trade.entryPos = lastbar;
				AddTradeOpenRecord(trade.type, ((QuoteData) quotes[lastbar]).time, Constants.ACTION_BUY, trade.POSITION_SIZE, trade.price);
				logger.warning(symbol + " " + CHART + " trade detected BUY at " + data.time + "@" + trade.price);

				trade.stop = Utility.getLow(quotes, lastHigh.pos, lastbar).low;
	...
	public Trade checkHigherHighLowerLowSetup3d(QuoteData data, Object[] quotesL)
	{
		Object[] quotes = getQuoteData();
		int lastbar = quotes.length - 1;
		int lastbarL = quotesL.length - 1;
	...
	public Trade checkHigherHighLowerLowSetup4(QuoteData data, Object[] quotesL)
	{
			...
			if ((direction == Constants.DIRECTION_DOWN) && (ema20L[lastbarL] < ema50L[lastbarL])
					&& ((((QuoteData) quotesL[lastbarL]).high > ema20L[lastbarL]) || (((QuoteData) quotesL[lastbarL - 1]).high > ema20L[lastbarL - 1])))
			{
			...
			else if ((direction == Constants.DIRECTION_UP) && (ema20L[lastbarL] > ema50L[lastbarL])
					&& ((((QuoteData) quotesL[lastbarL]).low < ema20L[lastbarL]) || (((QuoteData) quotesL[lastbarL - 1]).low < ema20L[lastbarL - 1])))
			{
	...
	public void trackTradeEntry(QuoteData data, Object[] quotesS, Object[] quotesL)
	{
		...
		if (trade != null)
		{
			...
			else if (Constants.TRADE_TOP.equals(trade.type))
				trackRSCTradeEntry2(data, quotesL);
		}
	}
	...
	public void trackHigherHighLowerLowEntry_latest(QuoteData data, Object[] quotesL)
	{
		// logger.info(symbol + " " + trade.type + " track trade entry " +  trade.detectTime);
		Object[] quotes = getQuoteData();
		int lastbar = quotes.length - 1;
				...
				*/
					...
					if ( phl_cur.curPos == lastbar )
					{
						...
						{	
							...
							{
								if (/*(prremptive_limit == true)&&*/((((QuoteData) quotes[phl_pre.curPos]).high - ((QuoteData) quotes[phl_pre.prePos]).high)  > 18 * PIP_SIZE) && (trade.limit1Placed == false))
								{
									logger.warning(symbol + " " + CHART + " detected for limit order " + data.time);
									trade.pullBackPos = phl_cur.pullBack.pos;
		
									trade.limitPrice1 = adjustPrice(((QuoteData) quotes[phl_cur.prePos]).high + 2*PIP_SIZE, Constants.ADJUST_TYPE_UP);
					...
					*/
					...
					if ( phl_cur.curPos == lastbar )
					{
						...
						{	
							...
							if (( phl_pre != null ))//&& (prremptive_limit == true))
							{
								if (/*(prremptive_limit == true)&&*/(((QuoteData) quotes[phl_pre.prePos]).low - ((QuoteData) quotes[phl_pre.curPos]).low > 18 * PIP_SIZE) && (trade.limit1Placed == false))
								{
								...
								// check 2: large break outs
								if ((breakout_limit == true) && ((((QuoteData) quotes[phl_cur.prePos]).low - data.low) > 10 * PIP_SIZE) && (trade.limit2Placed == false))
								{
									logger.warning(symbol + " " + CHART + " detected for limit order for large pull backs" + data.time);
									trade.pullBackPos = phl_cur.pullBack.pos;
	...
	public void trackHigherHighLowerLowEntry_org(QuoteData data, Object[] quotesL)
	{
		// logger.info(symbol + " " + trade.type + " track trade entry " +  trade.detectTime);
		Object[] quotes = getQuoteData();
		int lastbar = quotes.length - 1;
				...
				*/
					...
					if ( phl_cur.curPos == lastbar )
					{
						...
						{	
							...
							if (( phl_pre != null ))//&& (prremptive_limit == true))
							{
								if ((prremptive_limit == true) && ((((QuoteData) quotes[phl_pre.curPos]).high - ((QuoteData) quotes[phl_pre.prePos]).high)  > ENTRY_LARGE_BREAKOUT * PIP_SIZE))
								{
									logger.warning(symbol + " " + CHART + " detected for limit order " + data.time);
									trade.pullBackPos = phl_cur.pullBack.pos;
								...
								// check 2: large break outs
								if ((breakout_limit == true) && ((((QuoteData) quotes[phl_pre.prePos]).high - phl_pre.pullBack.low)  > ENTRY_LARGE_PULLBACK * PIP_SIZE))
								{
									// if the pull back is larger than 20 pips, put one
									// limit positions
									if ((data.high - (((QuoteData) quotes[phl_cur.prePos]).low) > ENTRY_LARGE_BREAKOUT * PIP_SIZE) && (trade.limit2Placed == false))
	...
	{
		//logger.info(symbol + " " + trade.type + " track trade entry " +  data.time);
		Object[] quotes1 = getQuoteData();
		QuoteData[] quotes = Arrays.copyOf(quotes1, quotes1.length, QuoteData[].class); 
		int lastbar = quotes.length - 1;
		//double[] ema20 = Indicator.calculateEMA(quotes, 20);
		int lastbarL = quotesL.length - 1;
		double[] ema20L = Indicator.calculateEMA(quotesL, 20);
		...
		if (trade.action.equals(Constants.ACTION_SELL))
		{
			...
			/*
			int startL = lastbarL;
			while(((QuoteData)quotesL[startL]).high >= ema20L[startL])
				startL--;
			int start = findPositionByHour(quotes, ((QuoteData)quotesL[startL]).time, 2 );
			if ( start == Constants.NOT_FOUND )
				return;
			start = Utility.getLow(quotes, start-5, start).pos;
			*/
			
			int start = Utility.getLow(quotes, lastbar - 36, lastbar - 3).pos;
			logger.warning(symbol + " " + CHART + " curve staring point is " + ((QuoteData)quotes[start]).time);
		...
		else if (trade.action.equals(Constants.ACTION_BUY))
		{
			...
			/*
			int startL = lastbarL;
			while(((QuoteData)quotesL[startL]).low <= ema20L[startL])
				startL--;
			int start = findPositionByHour(quotes, ((QuoteData)quotesL[startL]).time, 2 );
			if ( start == Constants.NOT_FOUND )
				return;
			start = Utility.getHigh(quotes, start-5, start).pos;
			*/

			
			int start = Utility.getHigh(quotes, lastbar - 36, lastbar - 3).pos;

			int end = Utility.getLow(quotes, start, lastbar).pos;
			...
			//for (int s = lastbar; s > start; s--)
			{
				...
				{
					logger.warning(symbol + " " + CHART + "@@@this entry low between " +
							 ((QuoteData)quotes[phl_cur.prePos]).time + "@" + ((QuoteData)quotes[phl_cur.prePos]).low + "  -  " +
							 ((QuoteData)quotes[phl_cur.curPos]).time + "@" + ((QuoteData)quotes[phl_cur.curPos]).low + " pullback@" +
							 phl_cur.pullBack.time + "-" + phl_cur.pullBack.high);		
	...
	public void trackHigherHighLowerLowEntry_bak_200211(QuoteData data, Object[] quotesL)
	{
		// logger.info(symbol + " " + trade.type + " track trade entry " +  trade.detectTime);
		Object[] quotes = getQuoteData();
		int lastbar = quotes.length - 1;
		...
		if (trade.action.equals(Constants.ACTION_SELL))
		{
			...
			for (int i = lastbar; i > start; i--)
			{
				...
				if (phl_cur != null)
				{
					logger.warning(symbol + " " + CHART + " " + data.time + "@@@this high between " + 
							   ((QuoteData)quotes[phl_cur.prePos]).time + "@" + ((QuoteData)quotes[phl_cur.prePos]).high + "  -  " +
							   ((QuoteData)quotes[phl_cur.curPos]).time + "@" + ((QuoteData)quotes[phl_cur.curPos]).high + " pullback@" +  phl_cur.pullBack.time + ":" +phl_cur.pullBack.low );
	...
	synchronized void checkTriggerMarketSell(double price)
	{
		...
		if ( price < trade.phl_cur.pullBack.low)
		{
			...
			*/
			...
			for (int j = trade.phl_cur.pullBack.pos + 1; j < lastbar; j++)
			{
				if (((QuoteData) quotes[j]).low < trade.phl_cur.pullBack.low)
				{
					logger.warning(symbol + " " + CHART + ((QuoteData) quotes[j]).time + " below pullback low of "
							+ ((QuoteData) quotes[trade.phl_cur.pullBack.pos]).time + " , trade missed");
					trade = null;
					return;
				}
			}
			...
			trade.detectTime = ((QuoteData) quotes[lastbar]).time;
			trade.pullBackPos = trade.phl_cur.pullBack.pos;
			...
			{	
				// place market order
				trade.orderId = placeMktOrder(Constants.ACTION_SELL, tradeSize);
				trade.price = (MODE == Constants.TEST_MODE)?trade.phl_cur.pullBack.low:price;   
				trade.entryPrice = trade.price;
				trade.status = Constants.STATUS_PLACED;
				trade.remainingPositionSize += tradeSize;
				trade.entryTime = ((QuoteData) quotes[lastbar]).time;
				...
				AddOpenRecord(((QuoteData) quotes[lastbar]).time, Constants.ACTION_SELL, tradeSize, trade.price);
				// AddTradeOpenRecord( trade.type,data.time, Constants.ACTION_SELL, trade.positionSize, trade.price);
				...
				//trade.targetPrice1 = adjustPrice(trade.price - 4 * FIXED_STOP * PIP_SIZE, Constants.ADJUST_TYPE_DOWN  );
				//trade.targetPos1 = POSITION_SIZE/2;
			...
			{
				// limit order
				trade.limitPrice1 = adjustPrice(trade.phl_cur.pullBack.low, Constants.ADJUST_TYPE_UP);
				trade.limitPos1 = trade.POSITION_SIZE;
				logger.warning(symbol + " " + CHART + " place limite entry order of " + trade.limitPrice1 + " on " + trade.limitPos1);
				trade.limitId1 = placeLmtOrder(Constants.ACTION_SELL, trade.limitPrice1, trade.limitPos1, null);

				trade.limit1Stop = Utility.getHigh(quotes, trade.phl_cur.pullBack.pos, lastbar).high;
	...
	synchronized void checkTriggerMarketBuy(double price)
	{
		...
		if ( price > trade.phl_cur.pullBack.high)
		{
			...
			for (int j = trade.phl_cur.pullBack.pos + 1; j < lastbar; j++)
			{
				if (((QuoteData) quotes[j]).high > trade.phl_cur.pullBack.high)
				{
			...
			{	
				// place order
				trade.orderId = placeMktOrder(Constants.ACTION_BUY, trade.POSITION_SIZE);
				trade.price = (MODE == Constants.TEST_MODE)?trade.phl_cur.pullBack.high:price;
				trade.entryPrice = trade.price;
				trade.status = Constants.STATUS_PLACED;
				trade.remainingPositionSize = trade.POSITION_SIZE;
				trade.entryTime = ((QuoteData) quotes[lastbar]).time;
				...
				AddOpenRecord(((QuoteData) quotes[lastbar]).time, Constants.ACTION_BUY, tradeSize, trade.price);
				// AddTradeOpenRecord( trade.type,data.time,
				// Constants.ACTION_BUY, trade.positionSize,
				// trade.price);
				...
				//trade.targetPrice1 = adjustPrice(trade.price + 4 * FIXED_STOP * PIP_SIZE, Constants.ADJUST_TYPE_UP  );
				//trade.targetPos1 = POSITION_SIZE/2;
			...
			{
				// limit order
				trade.limitPrice1 = adjustPrice(trade.phl_cur.pullBack.high, Constants.ADJUST_TYPE_DOWN);
				trade.limitPos1 = trade.POSITION_SIZE;
				logger.warning(symbol + " " + CHART + " place limite entry order of " + trade.limitPrice1 + " on " + trade.limitPos1);
				trade.limitId1 = placeLmtOrder(Constants.ACTION_BUY, trade.limitPrice1, trade.limitPos1, null);

				trade.limit1Stop = Utility.getLow(quotes, trade.phl_cur.pullBack.pos, lastbar).low;
	...
	public void trackTradeTarget(QuoteData data, Object[] quotesS, Object[] quotesL)
	{
		...
		if (trade != null)
			...
			exit123_3_c_org(data, quotesL, trade.entryPosL);
	...
	public void exit123_3_c_org(QuoteData data, Object[] quotesL, int tradeEntryPosL)
	{
		int lastbarL = quotesL.length - 1;
			...
			else
			{
				...
				if ((phls != null) && (phls.length >=2))
				{
					...
					if ((trade.profitTake1 == false) && ( pullback_cur > pullback_pre * 1.4 )) 
					{
						...
						trade.targetPrice1 = adjustPrice(((QuoteData) quotes[phl_cur.prePos]).low+PIP_SIZE, Constants.ADJUST_TYPE_DOWN);
						trade.targetPos1 = trade.remainingPositionSize;
						trade.targetId1 = placeLmtOrder(Constants.ACTION_BUY, trade.targetPrice1, trade.targetPos1, null);
						logger.warning(symbol + " " + CHART + " target order place " + trade.targetPrice1 + " " + trade.targetPos1 + " "
								+ data.time);
					...
					if ((trade.profitTake2 == false)
							&& (pullback_cur > FIXED_STOP * 2 * PIP_SIZE))
					{

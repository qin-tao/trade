...
public class SF108c extends SF implements EWrapper
{
	...
	protected SF108cTradeManager[][] tradeManager;
	...
	public static void main(String[] args)
	{
		...
		if ( "Test".equalsIgnoreCase(args[0]))
    	{
    		...
    		open = new SF108c(account, port, conn_id, market, Constants.TEST_MODE, lot1, lot2, lot3, null);
    	...
    	{	
    		...
    		open = new SF108c(account, port, conn_id, market, Constants.REAL_MODE, lot1, lot2, lot3, logdir);
    		open.run();
	...
	public SF108c( String account, int port, String ib_id, String trade_market, int mode, int lot1, int lot2, int lot3, String outputdir)
	{
		...
		createContract( 14, 514, "USD", "CASH", "IDEALPRO", "JPY", false, lot2,   0.01, 20, Constants.PRICE_TYPE_2,"U");
...
//		createContract( 17, 517, "CAD", "CASH", "IDEALPRO", "JPY", true,  lot2,   0.01, 20, Constants.PRICE_TYPE_2,Constants.TIME_ZONE_13);
		...
		if (( MODE == Constants.REAL_MODE ) || ( MODE == Constants.RECORD_MODE )) 
        {
			...
			logger.warning("SF120 started in " + MODE + " mode at port " + port);
        }
			...
			if (reqId == id_realtime[i])
			{
				...
				if (( tradeManager[i][TRADE_5M].trade != null ) && ( Constants.TRADE_EUR.equals(tradeManager[i][TRADE_5M].trade.type))) 
				{
					if (tradeManager[i][TRADE_5M].inEUTradingTime(hour, minute) && (tradeManager[i][TRADE_5M].trade.status.equals(Constants.STATUS_OPEN)))
						tradeManager[i][TRADE_5M].trackTradeEntry( data,prevData[i], tradeManager[i][TRADE_1M].getQuoteData(),tradeManager[i][TRADE_60M].getQuoteData());
			...
			String timeZone )
	{
        ...
        id_realtime[totalSymbolIndex] = idReal;
       	...
       	tradeManager[totalSymbolIndex][0] = new SF108cTradeManager(IB_ACCOUNT, m_client,contract, totalSymbolIndex, logger );
       	...
       	tradeManager[totalSymbolIndex][1] = new SF108cTradeManager(IB_ACCOUNT, m_client,contract, totalSymbolIndex, logger );
       	...
       	tradeManager[totalSymbolIndex][2] = new SF108cTradeManager(IB_ACCOUNT, m_client,contract, totalSymbolIndex, logger );
       	...
       	tradeManager[totalSymbolIndex][3] = new SF108cTradeManager(IB_ACCOUNT, m_client,contract, totalSymbolIndex, logger );
       	...
       	tradeManager[totalSymbolIndex][4] = new SF108cTradeManager(IB_ACCOUNT, m_client,contract, totalSymbolIndex, logger );
       	...
       	tradeManager[totalSymbolIndex][5] = new SF108cTradeManager(IB_ACCOUNT, m_client,contract, totalSymbolIndex, logger );
       	tradeManager[totalSymbolIndex][6] = new SF108cTradeManager(IB_ACCOUNT, m_client,contract, totalSymbolIndex, logger );
	...
	private void initialize()
	{
		...
		tradeManager = new SF108cTradeManager[TOTAL_SYMBOLS][TOTAL_TRADES];	

...
public class SF304TradeManager extends TradeManager2
{
	SF304 mainProgram;
	boolean CNT = false;
    String TIME_ZONE;
...
//	int firstRealTimeDataPosL = 0;
	...
	public SF304TradeManager()
	{
		super();
	}

	public SF304TradeManager(String account, EClientSocket client, Contract contract, int symbolIndex, Logger logger)
	{
	...
	public void checkOrderFilled(int orderId, int filled)
	{
		...
		else if (orderId == trade.targetId)
		{
			...
			else if (trade.remainingPositionSize < 0)
			{
				...
				String oca = null;//new Long(new Date().getTime()).toString();
				...
				if (Constants.ACTION_SELL.equals(prevTradeType))
				{
					...
					trade.remainingPositionSize = Math.abs(prevPosionSize);
	...
	//
	// //////////////////////////////////////////////////////////////////////////////////////////////////////////
	public Trade checkReallyBreakOut(QuoteData data, double lastTick_bid, double lastTick_ask )
	{
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);
		int lastbarL = quotesL.length - 1;
		...
		if ( lastUpPos > lastDownPos)
		{
			...
			// instead of touch 20MA, use pullback 3 bars
			int touch20MA = 0;
			for ( int i = start ; i < lastbarL; i++)
			{
				...
				{
					touch20MA = i+1;
					//touch20MA = Utility.getLow( quotesL, i+1, i+2).pos;
			...
			if ( firstBreakOut != null )
			{
				...
				if (( firstBreakOut.high - ema20L[firstBreakOut.pos]) < FIXED_STOP * PIP_SIZE )
					return null;
...
//				trade.lastLargeMoveEnd = lastLargeMoveEnd;
		...
		{	
			//info("check sell" + data.time);
			//if ("20120711  23:10:00".equals(data.time))
			...
			for ( int i = start ; i < lastbarL; i++)
			{
				...
				{
					touch20MA = i+1;
					//touch20MA = Utility.getHigh( quotesL, i+1, i+2).pos;
			...
			if ( firstBreakOut != null )
			{
				...
				if (( ema20L[firstBreakOut.pos] - firstBreakOut.low) < FIXED_STOP * PIP_SIZE )
					return null;
...
*/
	...
	public Trade checkPullBacktoBigMoves2_20(QuoteData data)
	{
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);
		int lastbarL = quotesL.length - 1;
		double[] ema20L = Indicator.calculateEMA(quotesL, 20);

		int lastUpPos = Pattern.findLastPriceConsectiveAboveOrAtMA(quotesL, ema20L, lastbarL, 20);
		int lastDownPos = Pattern.findLastPriceConsectiveBelowOrAtMA(quotesL, ema20L, lastbarL, 20);

		if ( lastUpPos > lastDownPos)
		{
			int prevDownPos = Pattern.findLastPriceConsectiveBelowMA(quotesL, ema20L, lastUpPos, 2);
			if ( prevDownPos == Constants.NOT_FOUND )
				prevDownPos = 0;
			
			int highestPos = Utility.getHigh( quotesL, prevDownPos, lastbarL).pos;
			if ( highestPos == lastbarL )
				return null;

			//System.out.println( prevDownPos + " " +  highestPos + " " + lastbarL);
			PushList pushList = PushSetup.getDown1PushList(quotesL, highestPos, lastbarL);
			if (( pushList != null ) && ( pushList.phls != null ) && ( pushList.phls.length <= 4 ) && ( pushList.phls.length >= 2 ))
			{	
				//System.out.println(pushList.toString(quotesL, PIP_SIZE));
				PushHighLow lastPush = pushList.phls[pushList.phls.length-1];
				PushHighLow prevPush = pushList.phls[pushList.phls.length-2];
			
				for ( int i = lastPush.prePos + 1; i < lastbarL; i++)
					if ( quotesL[i].low < quotesL[lastPush.prePos].low )
						return null; // not the first one
				
				//System.out.println(lastbarL + " " + lastPush.curPos );
				//if ( lastbarL != lastPush.curPos)
				//	return null;
				
				// look for a strong push
				/*System.out.println("check for strong push " + quotesL[lastbarL].time + " " + quotesL[prevPush.pullBack.pos].time + " " + quotesL[lastPush.prePos].time);
				if (quotesL[lastbarL].time.equals("20121212  12:00:00"))
				{
					System.out.println(pushList.toString(quotesL, PIP_SIZE));
					System.out.println("here");
				}*/
				Push strongPush = Utility.findLargestConsectiveDownBars(quotesL, prevPush.pullBack.pos, lastPush.prePos);
				if ( strongPush == null )
					return null;
				//System.out.println(strongPush + " " + strongPush.pushStart + strongPush.pushEnd);
				double strongPushSize = quotesL[strongPush.pushStart].open - quotesL[strongPush.pushEnd].close;
				if (( strongPushSize > 0.5 * FIXED_STOP * PIP_SIZE ) && ( lastPush.pullBackSize < strongPushSize * 0.6 ))
				{
					if  (findTradeHistory( Constants.ACTION_SELL, quotesL[highestPos].time) != null) 
						return null;
	
					System.out.println( symbol + " breakout SELL detected at " + quotesL[lastbarL].time + " TIP HIGH:" + quotesL[highestPos].time + " " + quotesL[lastPush.prePos].time + " " + quotesL[lastPush.curPos].time );
					createOpenTrade(Constants.TRADE_PBK, Constants.ACTION_SELL);
					trade.status = Constants.STATUS_DETECTED;
					trade.entryPrice = trade.triggerPrice = quotesL[lastPush.prePos].low;
					
					trade.pullBackStartL = highestPos; 
					trade.detectPos = lastbarL;
					trade.setFirstBreakOutTime(quotesL[highestPos].time);
					trade.setFirstBreakOutStartTime(quotesL[highestPos].time);
					trade.setTouch20MATime(lastPush.pullBack.time);
	...
	public void checkTrendBreakBigChart(QuoteData data)
	{
		QuoteData[] quotes240 = getQuoteData(Constants.CHART_240);
		...
		System.out.println(quotes240[lastbar240].time);
		/*
		PushHighLow phl = TrendAnalysis.detectLatestMomentumChange( quotes240, lastbar240, 1.5*FIXED_STOP * PIP_SIZE);
		if (phl != null )
		{
			System.out.println(symbol + " " + ((phl.direction == Constants.DIRECTION_UP)?"UP  ":"DOWN") + " detected at " + quotes240[lastbar240].time + " push:"+ 
		        quotes240[phl.pushStart].time + " "	+ quotes240[phl.prePos].time);
		}*/
		
		boolean detected = TrendAnalysis.detectLatestMomentumChange2( quotes60, 0.5*FIXED_STOP * PIP_SIZE);
		if (detected == true )
		{
			//System.out.println(symbol + " DowN " + " detected at " + quotes60[lastbar60].time );
		}
...
//		}
	...
	public Trade checkPullBacktoBigMoves3(QuoteData data)
	{
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);
		QuoteData[] quotes240 = getQuoteData(Constants.CHART_240);
		int lastbarL = quotesL.length - 1;
		...
		{
			int prevDownPos = Pattern.findLastPriceConsectiveBelowMA(quotesL, ema20L, lastbarL, 2);
			int lowestPoint = Utility.getLow( quotesL, prevDownPos-15, lastbarL).pos;
			...
			PushList pushList = PushSetup.getUp2PushList(quotesL, lowestPoint, lastbarL);
			...
			{	
				...
				if (( lastBreakOut1 < 0.15 * FIXED_STOP * PIP_SIZE) ||( pullback  > 1.5 * FIXED_STOP * PIP_SIZE)) 
				{
					System.out.println( symbol + " breakout SELL detected at " + quotesL[lastbarL].time + " TIP HIGH:" + quotesL[lowestPoint].time + " " + quotesL[phls[0].prePos].time + " " + quotesL[phls[0].curPos].time );
					createOpenTrade(Constants.TRADE_PBK, Constants.ACTION_SELL);
					trade.status = Constants.STATUS_DETECTED;
					trade.entryPrice = trade.triggerPrice = quotesL[lastPush.prePos].high;
					
					trade.pullBackStartL = lowestPoint; 
					trade.detectPos = lastbarL;
					trade.setFirstBreakOutTime(quotesL[lowestPoint].time);
					trade.setFirstBreakOutStartTime(quotesL[lowestPoint].time);
					trade.setTouch20MATime(quotesL[lastPushIndex].time);
					enterMarketPosition(trade.entryPrice);
					return trade;
				}
			}
	...
	public Trade checkPullBacktoBigMoves(QuoteData data)
	{
		...
		if ((quotesL[lastbarL-2].close > ema10L[lastbarL-2]) && (quotesL[lastbarL-1].high < quotesL[lastbarL-2].high))
		{
			...
			if (( quotesL[lastPush.pushEnd].close - quotesL[lastPush.pushStart].open ) > 1.5 * FIXED_STOP * PIP_SIZE )
			{
				int lastDownPos = Pattern.findLastPriceConsectiveBelowMA(quotesL, ema10L, lastbarL, 2);

				if  (findTradeHistory( Constants.ACTION_BUY, quotesL[lastDownPos].time) != null) 
					return null;

				System.out.println(symbol + " " + CHART + " " + " BUY detected " + quotesL[lastbarL].time + " bigMove:" + quotesL[lastPush.pushStart].time + "-" + quotesL[lastPush.pushEnd].time + " " + quotesL[lastPush.pushStart].open + ":" + quotesL[lastPush.pushEnd].close);
				createOpenTrade(Constants.TRADE_PBK, Constants.ACTION_BUY);
				trade.status = Constants.STATUS_DETECTED;
				trade.entryPrice = trade.triggerPrice = quotesL[pushTip].high;
				
				trade.pullBackStartL = pushTip; 
				trade.detectPos = lastbarL;
				trade.setFirstBreakOutTime(quotesL[pushTip].time);
				trade.setFirstBreakOutStartTime(quotesL[lastDownPos].time);
		...
		else if ((quotesL[lastbarL-2].close < ema10L[lastbarL-2]) && (quotesL[lastbarL-1].low > quotesL[lastbarL-2].low))
		{
			...
			Push lastPush = Utility.findLargestConsectiveDownBars(quotesL, lastbarL-12, lastbarL-2);
			if (( quotesL[lastPush.pushStart].open - quotesL[lastPush.pushEnd].close ) > 1.5 * FIXED_STOP * PIP_SIZE )
			{
				int lastUpPos = Pattern.findLastPriceConsectiveAboveMA(quotesL, ema10L, lastbarL, 2);

				if  (findTradeHistory( Constants.ACTION_SELL, quotesL[lastUpPos].time) != null) 
					return null;

				System.out.println(symbol + " " + CHART + " " + " SELL detected " + quotesL[lastbarL].time + " bigMove:" + quotesL[lastPush.pushStart].time + "-" + quotesL[lastPush.pushEnd].time );
				createOpenTrade(Constants.TRADE_PBK, Constants.ACTION_SELL);
				trade.status = Constants.STATUS_DETECTED;
				trade.entryPrice = trade.triggerPrice = quotesL[pushTip].low;
				
				trade.pullBackStartL = pushTip; 
				trade.detectPos = lastbarL;
				trade.setFirstBreakOutTime(quotesL[pushTip].time);
				trade.setFirstBreakOutStartTime(quotesL[lastUpPos].time);
				trade.setTouch20MATime(quotesL[lastbarL-1].time);
				return trade;
...
*/
	...
	public void trackTradeTarget(QuoteData data, int field, double currPrice)
	{
		...
		{	
			...
			{
				//exit123_new_reversal( data );  
				exit123_new9c4( data ); //previous default  
	...
	public void exit123_new_reversal( QuoteData data )
	{
		QuoteData[] quotes = getQuoteData(Constants.CHART_15);
		int lastbar = quotes.length - 1;
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);;
	...
	public void exit123_new9_checkReversalsOrEalyExit( QuoteData data )
	{
		QuoteData[] quotes = getQuoteData(Constants.CHART_15);
		int lastbar = quotes.length - 1;
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);;
		...
		int firstBreakOutPosL = trade.getFirstBreakOutStartPosL(quotesL);
		int tradeEntryPosL = trade.getEntryTimePosL(quotesL);
		//int tradeEntryPos = Utility.findPositionByMinute( quotes, trade.entryTime, Constants.BACK_TO_FRONT);

		//BigTrend bt = determineBigTrend( quotesL);
		BigTrend bt = determineBigTrend_v3(quotesL);
		Push bigTrendLastPush = bt.pushes.elementAt(bt.lastPushIndex);
		System.out.println(bt.toString(quotesL));
		//BigTrend bt = determineBigTrend2( quotes240);

		if (Constants.ACTION_SELL.equals(trade.action))
		{
			int prevUp = Pattern.findLastPriceConsectiveAboveMA(quotesL, ema20L, firstBreakOutPosL, 2);
			int prevUpStart = Pattern.findLastPriceConsectiveAboveMA(quotesL, ema20L, prevUp, 1);
			prevUp = Utility.getLow( quotesL, prevUpStart, prevUp).pos;
			
			//trade type:  Continuation
			/*
			boolean againstTrend = false;
			if ( bt.lastPushIndex == bt.lastTrendPushIndex ) && bigTrendLastPush.direction == Constants.DIRT_UP
			{
				System.out.println("in a trend");
			}
			else if ( bt.lastTrendPushIndex == ( bt.lastPushIndex - 1) )
			{
				if ( bigTrendLastPush.duration < 48 )
				{
					
				}
			}*/
			
			// need to do an analysis if it is againt the trend
			if ( Constants.DIRT_UP.equals(bt.direction) ||  Constants.DIRT_UP_SEC_2.equals(bt.direction)) 
			{
				for ( int i = lastbarL; i > prevUp; i--)
				{	
					PushList pushList = Pattern.findPast1Lows1(quotesL, prevUp, i);
					...
					{	
						...
						double lastPullbackSize = lastPush.pullBack.high - quotesL[lastPush.prePos].low ;

						if ( lastPullbackSize > FIXED_STOP * PIP_SIZE )
						{
							//double reversePrice = quotesL[lastPush.prePos].low;
							//closeReverseTrade( true, reversePrice, POSITION_SIZE );
							//return;
						}
						else if ( pushSize > 3 )
						{
						}
						else if (( pushSize >= 2 ) && ( lastPush.pullBackSize > 1.5 * FIXED_STOP * PIP_SIZE))
						{
			...
			//  look to reverse if it goes against me soon after entry
			double lowestPointAfterEntry = Utility.getLow(quotesL, tradeEntryPosL, lastbarL).low;
			if ( tip_reverse && ((( trade.entryPrice - lowestPointAfterEntry) < FIXED_STOP * PIP_SIZE * 0.3 )))     
			{
			...
			// check after stopped out
			
			
		}
		else if (Constants.ACTION_BUY.equals(trade.action))
		{
			int prevDown = Pattern.findLastPriceConsectiveBelowMA(quotesL, ema20L, firstBreakOutPosL, 2);
			int prevDownStart = Pattern.findLastPriceConsectiveAboveMA(quotesL, ema20L, prevDown, 1);
			prevDown = Utility.getLow( quotesL, prevDownStart, prevDown).pos;

			Vector<Push> pushes = bt.pushes;
			int pushSize = pushes.size();
			...
			{
				for ( int i = lastbarL; i > prevDown; i--)
				{	
					PushList pushList = Pattern.findPast1Highs1(quotesL, prevDown, i);
					if (( pushList.phls != null ) && ( pushList.phls.length > 0 )) // latest
					{	
						PushHighLow[] phls = pushList.phls;
						pushSize = pushList.phls.length;
						...
						if ( lastPullbackSize > FIXED_STOP * PIP_SIZE )
						{
							//double reversePrice = quotesL[lastPush.prePos].high;
							//closeReverseTrade( true, reversePrice, POSITION_SIZE );
							//return;
						}
						else if ( pushSize > 3 )
			...
			//  look to reverse if it goes against me soon after entry
			double highestPointAfterEntry = Utility.getHigh(quotesL, tradeEntryPosL, lastbarL).high;
			if (tip_reverse && ((( highestPointAfterEntry - trade.entryPrice) < FIXED_STOP * PIP_SIZE *0.3 ))            )/*      || 
...
//					info("Wave2 touch point is " + quotesL[wave2PtL].time);
	...
	protected void potentialExit( double price, int size )
	{
	}

...
public class SF108cTradeManager extends TradeManager2 implements Serializable
{
	...
	protected SF108cTradeManager largerTimeFrameTraderManager;
	...
	public SF108cTradeManager()
	{
		super();
	}

	public SF108cTradeManager(String account, EClientSocket client, Contract contract, int symbolIndex, Logger logger)
	{
...
/*
	...
	public boolean inJPTakeProfitTime( int hour, int min )
	{
		...
		if ( TIME_ZONE.indexOf("A") != -1 )
		{	
			//if (( minute >= 1260) || ( minute < 780 ))  //  after 9 pm or < 11am
	...
	public Trade checkHigherHighLowerLowSetup3d(QuoteData data, QuoteData prevData, QuoteData[] quotesL)
	{
		QuoteData[] quotes = getQuoteData();
		int lastbar = quotes.length - 1;
		int lastbarL = quotesL.length - 1;
				...
				&& (( quotesL[lastbarL].high > ema20L[lastbarL]) || ( quotesL[lastbarL - 1].high > ema20L[lastbarL - 1])))
		{
			...
			// now it is above 20MA
			startL = Pattern.findLastPriceConsectiveAboveMA(quotesL, ema20L, startL, 1);
			if ( startL != Constants.NOT_FOUND )
			{
				int startL0 = startL-1;
				while ((startL0 > 0) && (quotesL[startL0].low > ema20L[startL0]))  
					startL0--;
					...
					{	
						...
						if ( phl_pre != null )
						{
							if (( data.high > phl_pre.pullBack.high) && ( prevData != null ) && ( prevData.high < phl_pre.pullBack.high))
							{
								...
								// return null;
								//logger.warning(symbol + " " + CHART + " breaks higher high lower low, remove trade, place break order");
								createOpenTrade(Constants.TRADE_EUR, Constants.ACTION_BUY);
								trade.detectTime = quotes[lastbar].time;
								if (previousTradeExist(Constants.TRADE_EUR, Constants.ACTION_BUY))
									return null;
		
								trade.POSITION_SIZE = trade.remainingPositionSize = POSITION_SIZE;
								trade.entryPrice = trade.price = phl_pre.pullBack.high;
								trade.entryTime = ((QuoteData) quotes[lastbar]).time;
				...
				&& ((quotesL[lastbarL].low < ema20L[lastbarL]) || ( quotesL[lastbarL - 1].low < ema20L[lastbarL - 1])))
		{
			...
			// now it is above 20MA
			startL = Pattern.findLastPriceConsectiveBelowMA(quotesL, ema20L, startL, 1);
			if ( startL != Constants.NOT_FOUND )
			{
				int startL0 = startL-1;
				while ((startL0 > 0) && (quotesL[startL0].high < ema20L[startL0]))
					startL0--;
					...
					{	
						...
						if ( phl_pre != null )
						{
							if (( data.low < phl_pre.pullBack.low) && ( prevData != null ) && ( prevData.low > phl_pre.pullBack.low))
							{
								...
								createOpenTrade(Constants.TRADE_EUR, Constants.ACTION_SELL);
								trade.detectTime = quotes[lastbar].time;
								if (previousTradeExist(Constants.TRADE_EUR, Constants.ACTION_SELL))
									return null;
		
								trade.POSITION_SIZE = trade.remainingPositionSize = POSITION_SIZE;
								trade.entryPrice = trade.price = phl_pre.pullBack.low;
								trade.entryTime = ((QuoteData) quotes[lastbar]).time;
...
*/
	...
	public Trade checkBreakingHigherHighLowerLowSetup(QuoteData data, QuoteData prevData, QuoteData[] quotesL)
	{
		...
		tao.trading.SlimJim sj = calculateChannelSlimJim( quotesL, lastbarL, 10, 2 * FIXED_STOP );
		...
		if ( sj != null )
		{
			if ( sj.direction == Constants.DIRECTION_UP )
			{
				logger.warning(symbol + " " + CHART + " slim jim channel UP detected at " + quotesL[lastbarL].time );
				createOpenTrade(Constants.TRADE_EUR, Constants.ACTION_SELL);
				trade.detectTime = quotes[lastbar].time;
				trade.startL = sj.phl.pullBack.pos;
				//if (!previousTradeExist(trade.type, Constants.ACTION_SELL))
					return trade;
	
			}
			else if ( sj.direction == Constants.DIRECTION_DOWN )
			{
				logger.warning(symbol + " " + CHART + " slim jim channel DOWN detected at " + quotesL[lastbarL].time );
				createOpenTrade(Constants.TRADE_EUR, Constants.ACTION_BUY);
				trade.detectTime = quotes[lastbar].time;
				trade.startL = sj.phl.pullBack.pos;
				//if (!previousTradeExist(trade.type, Constants.ACTION_BUY))
	...
	public Trade checkBreakingHigherHighLowerLowSetup_org(QuoteData data, QuoteData prevData, QuoteData[] quotesL)
	{
		QuoteData[] quotes = getQuoteData();
		int lastbar = quotes.length - 1;
		int lastbarL = quotesL.length - 1;
		...
		if ( upPos > downPos)
		{
			...
			int start = Utility.getLow(quotesL, upPos-15, upPos).pos;
			...
			{	
				...
				if ( phl_pre != null )
				{
					...
					if (( data.low < phl_pre.pullBack.low) && (( prevData != null ) && (prevData.low > phl_pre.pullBack.low)))       
					{	
						//double lastHigh = Utility.getHigh( quotesL, phl_pre.pullBack.pos, lastbarL).high;
						//if  (( lastHigh - phl_pre.pullBack.low ) > 2 * FIXED_STOP * PIP_SIZE )
						//	return null;
						//if (previousTradeExist(Constants.ACTION_SELL, phl_pre.pullBack.pos))
						//	return null;

						trade = new Trade(symbol);
						
						logger.warning(symbol + " " + CHART + " minute Higher high break up SELL at " + data.time );
						logger.warning(symbol + " " + CHART + "last high between " + ((QuoteData)quotesL[phl_pre.prePos]).time + "@" + ((QuoteData)quotesL[phl_pre.prePos]).high + "  -  " + ((QuoteData)quotesL[phl_pre.curPos]).time + "@" + ((QuoteData)quotesL[phl_pre.curPos]).high + " pullback@" + phl_pre.pullBack ); 
						...
						trade.stop = trade.price + 1.5 * FIXED_STOP * PIP_SIZE;
		...
		else if ( downPos > upPos )
		{
			...
			// looking for breaking up trend
			int start = Utility.getHigh(quotesL, downPos-15, downPos).pos;
			for ( int i = lastbarL; i > start; i--)
			{	
				...
				if ( phl_pre != null )
				{
					...
					if (( data.high > phl_pre.pullBack.high) )       
					{	
						//if (previousTradeExist(Constants.ACTION_BUY, phl_pre.pullBack.pos))
						//	return null;
						//double lastLow = Utility.getLow( quotesL, phl_pre.pullBack.pos, lastbarL).low;
						//if  ((phl_pre.pullBack.high - lastLow ) > 2 * FIXED_STOP * PIP_SIZE )
						//	return null;
	...
	//
	// ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	public void trackTradeEntry(QuoteData data, QuoteData prevData, QuoteData[] quotesS, QuoteData[] quotesL)
	{
		...
		if (trade != null)
		{
			//if (Constants.TRADE_EUR.equals(trade.type))
				trackHigherHighLowerLowEntry(data, prevData, quotesL);
		}
	}
	...
	public void trackHigherHighLowerLowEntry(QuoteData data, QuoteData prevData, QuoteData[] quotesL)
	{
		// logger.info(symbol + " " + trade.type + " track trade entry " +  trade.detectTime);
		QuoteData[] quotes = getQuoteData();
		int lastbar = quotes.length - 1;
		...
		if (trade.action.equals(Constants.ACTION_SELL))
		{
			if (( data.low < quotesL[trade.startL].low ) && ( prevData.low >= quotesL[trade.startL].low))
			{
				enterSellPosition( data, quotesL[trade.startL].low);
				return;
			}
			/*
			int start = findPositionByMinute(quotes, trade.detectTime, Constants.BACK_TO_FRONT );
			for (int s = lastbar; s > start; s--)
			{
				...
				if (phl_cur != null)
				{
					...
					{	
						enterSellPosition( data, phl_cur.pullBack.low);
						return;
					}
				}
			}*/
		...
		{
			if (( data.high > quotesL[trade.startL].high ) && ( quotesL[trade.startL].high <= quotesL[trade.startL].high))
			{
				enterBuyPosition( data, quotesL[trade.startL].high);
				return;
			}
			...
			for (int s = lastbar; s > start; s--)
			{
				...
				if (phl_cur != null)
				{
					...
					{	
						enterBuyPosition( data, phl_cur.pullBack.high);
						return;
					}
				}
			}*/
...
*/
	...
	*/
	...
	public tao.trading.SlimJim calculateChannelSlimJim( QuoteData[] quotes, int lastbar, int minBars, int channelHeight )
	{
		tao.trading.SlimJim sj = new tao.trading.SlimJim();
		
		while ( true )
		{
			for ( int i = lastbar; i >= lastbar-minBars; i--)
			{
				if ((((QuoteData)quotes[i]).high - ((QuoteData)quotes[i]).low) > channelHeight * PIP_SIZE )
					return null;
				
				if (((QuoteData)quotes[i]).high > sj.slimJimHigh)
				{
					sj.slimJimHigh = ((QuoteData)quotes[i]).high;
					sj.slimJimHighPos = i;
				}
				
				if (((QuoteData)quotes[i]).low < sj.slimJimLow)
				{
					sj.slimJimLow = ((QuoteData)quotes[i]).low;
					sj.slimJimLowPos = i;
				}
			}
			
			if (sj.slimJimHigh - sj.slimJimLow > channelHeight * PIP_SIZE )
				return null;
			
			//logger.warning(symbol + " " + CHART + " slim channel detected between" + quotes[lastbar-minBars].time + " : " + quotes[lastbar].time);
			for ( int i = lastbar; i > lastbar-minBars; i--)
			{	
				PushHighLow phl_pre = findLastNHigh( quotes, lastbar-minBars, i, 1 );
				if ( phl_pre != null )
				{	
					sj.direction = Constants.DIRECTION_UP;
					sj.phl = phl_pre;
					return sj;
				}

				phl_pre = findLastNLow( quotes, lastbar-minBars, i, 1 );
				if ( phl_pre != null )
				{	
					sj.direction = Constants.DIRECTION_DOWN;
					sj.phl = phl_pre;
					return sj;
				}
			}

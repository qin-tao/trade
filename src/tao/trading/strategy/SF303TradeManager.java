...
import tao.trading.BallingerBand;
...
public class SF303TradeManager extends TradeManager2
{
	SF303 mainProgram;
	boolean CNT = false;
    String TIME_ZONE;
...
//	int firstRealTimeDataPosL = 0;
	...
	public SF303TradeManager()
	{
		super();
	}

	public SF303TradeManager(String account, EClientSocket client, Contract contract, int symbolIndex, Logger logger)
	{
	...
	public Trade checkBreakOutFromNarrowRange(QuoteData data)
	{
		QuoteData[] quotesL = getQuoteData(Constants.CHART_60);
		QuoteData[] quotes240 = getQuoteData(Constants.CHART_240);
		int lastbarL = quotesL.length - 1;
		...
		BallingerBand[] bbands = Indicator.calculateBallingerBand( quotesL );
		double avgBarSize60 = Utility.averageBarSize(quotesL);


		if (((bbands[lastbarL].upperBB - bbands[lastbarL].lowerBB) < 3 * avgBarSize60) && 
		    ((bbands[lastbarL-1].upperBB - bbands[lastbarL-1].lowerBB) > 3 * avgBarSize60) &&
		    ((bbands[lastbarL-2].upperBB - bbands[lastbarL-2].lowerBB) > 3 * avgBarSize60))
		{
			int start = lastbarL;
			
			logger.warning(symbol + " " + CHART + " " + " trade detected " + quotesL[lastbarL].time);
			BigTrend bt = determineBigTrend( quotesL);
...
*/
		...
		int lastUpPos = Pattern.findLastPriceConsectiveAboveMA(quotes240, ema50_240, lastbar240, 8);
		int lastDownPos = Pattern.findLastPriceConsectiveBelowMA(quotes240, ema50_240, lastbar240,8 );

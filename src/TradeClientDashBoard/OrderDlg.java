...
//	protected Contract contract;
    ...
    public OrderDlg( SampleFrame owner, String id, String symbol, String secType, String exchange, String currency ) {
        super( owner, true);
...
*/
    ...
    void onSellTop()
    {
    	command = Constants.ACTION_SELL;
		Trade tt = new Trade(symbol);
		tt.action = Constants.ACTION_SELL;
		tt.status = Constants.STATUS_OPEN;
		tt.POSITION_SIZE = positionSize;
		tradeManager.setTrade(tt);

		m_messagePanel.add(command + " command received");
    	//onPlaceTrade(Constants.ACTION_SELL);
    }
    
    void onBuyBottom()
    {
    	command = Constants.ACTION_BUY;
		Trade tt = new Trade(symbol);
    ...
    void onPlaceTrade( String action )
    {
   		if ( tradeManager.getTrade() == null )
   		{
       		...
       		tt.status = Constants.STATUS_OPEN;
       		tt.reEnter = 3;
       		//tt.positionSize = calculatePositionSize();
			...
			int permId, int parentId, double lastFillPrice, int clientId, String whyHeld)
	{
		...
		*/
		...
		if ( trade != null )
		{
			if ( "Filled".equals(status) && (orderId == trade.stopMarketOrderId))
			{	
				m_messagePanel.add("Trade " + symbol + " "  + " filled");
				...
				int beginSize = size >= 11? size-11:0;
				
				if ( Constants.ACTION_SELL.equals(trade.action))
				{	
					trade.stop = tradeManager.getHigh(quotes.toArray(), beginSize, size-1);
					tradeManager.placeStopOrder(Constants.ACTION_BUY, new Double(decf.format(trade.stop)), null );
				}
				else if ( Constants.ACTION_BUY.equals(trade.action))
				{
					trade.stop = tradeManager.getLow(quotes.toArray(), beginSize, size-1);
					tradeManager.placeStopOrder(Constants.ACTION_SELL, new Double(decf.format(trade.stop)), null );
				}
			}
		}
		}
			...
			long volume, double wap, int count)
	{
		...
		updateLastQuoteEntry( quotes1m, data );
		updateLastQuoteEntry( quotes3m, data );

		if ( second == 55) // this is the last bar for the minute
		{
			if ( chart_period == 1 )
			{
				adjustPendingOrder(quotes1m);
				createNewQuoteEntry( quotes1m, data );
			}
			...
			if ( (minute+1) % 3 == 0 )
			{
				if ( chart_period == 3 )
				{
					adjustPendingOrder(quotes3m);
					createNewQuoteEntry( quotes3m, data );
				}
			}
			...
			if ( trade != null ) 
			{
				...
				else if (( trade.status == Constants.STATUS_PLACED))
				{
					createNewQuoteEntry( trade.exitQuotes, data );
				}
			...
			/*		if ( minute == 0 )
				...
				/*
				double[] ema200 = indicator.calculateEMA(quotes[i], 200);
				double[] ema60_5 = indicator.calculateEMA(quotes60M[i], 5);
				double[] ema60_20 = indicator.calculateEMA(quotes60M[i], 20);

				// this is for better entry
				if ( trade == null )
				{
					if  ( trade_timeout == false )
					{	
						if (direction[i] != -1 ) 
						{
							if (( trade = tradeManager[i].calculateStretchHigh( quotes[i], quotes60M[i], ema200, ema60_5, ema60_20)) != null )
							{
						...
						if (direction[i] != 1 ) 
						{
							if (( trade = tradeManager[i].calculateStretchLow( quotes[i], quotes60M[i], ema200, ema60_5, ema60_20)) != null )
							{
								tradeManager[i].trade = trade;
								logger.info(contracts[i].m_symbol+"."+contracts[i].m_currency+ " " + quotes[i].size() + " trade placed");
							}
				...
				{
					// trade already exists
					if (trade.status.equals(Constants.STATUS_OPEN) || trade.status.equals(Constants.STATUS_ONE_MORE_CLEARED))
					{
						updateLastQuoteEntry( trade.entryQuotes, data );
						//logger.info(contracts[i].m_symbol+"."+contracts[i].m_currency + "trackTradeEntry");
						tradeManager[i].trackTradeEntry(data, ema200);
					}
					else if (trade.status.equals(Constants.STATUS_ONE_MORE))
					{
						updateLastQuoteEntry( trade.entryQuotes, data );
						//logger.info(contracts[i].m_symbol+"."+contracts[i].m_currency + "trackTradeEntry");
						tradeManager[i].trackOneMoreEntry(data, ema200);
					}
					else if (trade.status.equals(Constants.STATUS_PLACED))
					{
						...
						tradeManager[i].trackTradeTarget(data, ema200, quotes[i]);
					}
					else if (trade.status.equals(Constants.STATUS_EXITING))
					{
						...
						// this is to check whether it needs any "stop and reversal"
						//logger.info(contracts[i].m_symbol+"."+contracts[i].m_currency + "trackTradeExit");
						tradeManager[i].trackTradeExit(quotes[i], ema200 );
					}
				}
			}*/
		}
...
*/
	...
	private void sellTrigger( double price, Vector<QuoteData> quotes )
	{
		int size = quotes.size();
		if ( price < quotes.elementAt(size-2).low ) 
		{
			Trade tt = new Trade(symbol);
			tt.action = Constants.ACTION_SELL;
			int beginSize = size >= 11? size-11:0;
			tt.stop = tradeManager.getHigh(quotes1m.toArray(), beginSize, size-1);
			tt.POSITION_SIZE = positionSize;
			tradeManager.setTrade(tt);

			//trade.
			tradeManager.placeMktOrder();
			//tradeManager.placeStopOrder(Constants.ACTION_BUY, new Double(decf.format(trade.stop+STOP_FACTOR*pipSize)), null );
			command = null;
			m_messagePanel.add("Trade " + symbol + " " + tt.action + " " + tt.POSITION_SIZE + " placed - stop set at:" + tt.stop);
		}
	}
	
	private void buyTrigger( double price, Vector<QuoteData> quotes )
	{
		int size = quotes.size();

		if ( price > quotes.elementAt(size-2).high )
		{
			Trade tt = new Trade(symbol);
			tt.action = Constants.ACTION_BUY;
			int beginSize = size >= 11? size-11:0;
			tt.stop = tradeManager.getLow(quotes.toArray(), beginSize, size-1);
			tt.POSITION_SIZE = positionSize;
			tradeManager.setTrade(tt);
	...
	protected  void adjustPendingOrder( Vector<QuoteData> quotes)
	{
		Trade trade = tradeManager.getTrade();
		...
		if (( trade != null ) && ( trade.status.equals(Constants.STATUS_OPEN)))
		{
			if ( trade.stopMarketOrderId != 0 )
				tradeManager.cancelOrder(trade.stopMarketOrderId);
			
			if ( Constants.ACTION_BUY.equals(trade.action))
			{
				trade.stopMarketOrderId = tradeManager.placeStopOrder(Constants.ACTION_BUY, quotes.lastElement().high);
			}
			else if ( Constants.ACTION_SELL.equals(trade.action))
			{
				trade.stopMarketOrderId = tradeManager.placeStopOrder(Constants.ACTION_SELL, quotes.lastElement().low);
			}
		}
